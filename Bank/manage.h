...
class manage_loan : public QWidget
{
    Q_OBJECT
public:
    explicit manage_loan(QWidget *parent = nullptr);

    explicit manage_loan(QVector<loan>, int, int, int);
public slots:
    void check_accept();
    void check_Refuse();
    void check_back();
...
private:
    ...
    int id_manager;
    QVector<loan> loans;
    QLineEdit *lname, *lfamily ,*lNational_Code,*lReceived_account_number ,*lPayment_account_number , *lInstallment_period , *lbudget;
    QLabel *name, *family, *National_Code, *Received_account_number ,*Payment_account_number , *Installment_period , *budget;
    QFormLayout *mainlayout;
    QHBoxLayout *forpush, *fornext_back;
    QPushButton *Refuse, *accept;
    QPushButton *next, *back, *panel;
